...
/*--------------------------------------------------------------*/
{
  ...
  char dir[80], filename[60], nombre[40];
  char dump[60], outdir[100], fname[100];
  float minv = 1000000;



  if((fp1 = fopen("filelist_SAC", "r"))==NULL) {
    printf("cannot open filelist_SAC\n");
    exit(1);
  }
 
  if((fp2 = fopen("filelist_SAC_out", "w"))==NULL) {
    printf("cannot open filelist_SAC\n");
    exit(1);
  }
 ...
 fgets(filename, 60, fp1); /*name of month.lst */
