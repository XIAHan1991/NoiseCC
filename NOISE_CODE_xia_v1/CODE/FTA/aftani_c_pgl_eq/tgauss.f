...
c function [ss] = tgauss(fsnr,gt0,t0,dw,dt,n,seis);
     ...
     *                  ss)
      ...
      integer*4 nc,ii,nnl,nnnl,nnr,nnnr,
     *		nleft,nright,left(100),right(100)
...
c find global max, sm, and index ism
      ...
      do i = 1,n
         ...
         ss(i) = seis(i)
      enddo
      write(*,*) 'Distance between maximas=',
     *		gt0-(ism-1)*dt-t0,' in sec,',
     * ' Spectra point= ',ism
      write(*,*) 'gt0 ism dt t0',gt0, ism, dt, t0
...
c left side 
      ...
      do i = ism-1,2,-1     
          dl = smax(i)-smax(i-1)
          dr = smax(i+1)-smax(i)
          if((dl.lt.dzer.and.dr.ge.dzer).or.
     *		(dl.le.dzer.and.dr.gt.dzer)) then
              nleft = nleft+1
              left(nleft) = i
              vleft(nleft) = smax(i)
          endif
