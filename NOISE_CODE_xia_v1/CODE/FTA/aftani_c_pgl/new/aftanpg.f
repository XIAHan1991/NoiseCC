...
c
     ...
     *           tresh,ffact,perc,npoints,taperl,nfin,
     *           fsnr,nphpr,phprper,
     *           phprvel,nfout1,arr1,nfout2,arr2,tamp,nrow,
     *           ncol,amp,ierr);
c======================================================================
...
c nfin    - starting number of frequencies, nfin <= 64,(integer*4)
...
c======================================================================
      ...
      real*8    t0,dt,delta,vmin,vmax,tmin,tmax,tresh
      real*8	ffact,ftrig(100)
      real*8    fsnr
      real*4    sei(32768)
      double complex dczero,s(32768),sf(32768),fils(32768),tmp(32768)
      real*8    grvel(100),tvis(100),ampgr(100),om(100)
      real*8	per(100),tim(100)
      real*8    pha(32768,64),amp(32768,64),ampo(32768,64)
...
c ---
      integer*4 njump,nijmp,i,ii(100),nii,ijmp(100)
      integer*4 ki,kk,istrt,ibeg,iend,ima
      real*8    dmaxt,wor,per2(100),om1(100),snr(100)
      real*8 	snr1(100),snrt(100)
      real*8    wdth(100),wdth1(100),wdtht(100)
      integer*4 ip,iflag,ierr1,nindx,imax,iimax
      integer*4 ipos,ist,ibe,nfout2,indx(100)
...
c tapering both ends of seismogram
      call taper(max0(nb,ntapb+1),min0(ne,n-ntape),
     &  n,sei,ntapb,ntape,s,ns)
